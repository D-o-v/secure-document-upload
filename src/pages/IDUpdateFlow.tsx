...
export default function IDUpdateFlow({ onBack }: IDUpdateFlowProps) {
  ...
  const handleFormSubmit = useCallback(async (data: IDUpdateFormValues) => {
    setIsSubmitting(true);
    setStep("terms");
    setIsSubmitting(false);
  }, []);

  const [formData, setFormData] = useState<IDUpdateFormValues | null>(null);

  const handleFormSubmitCapture = useCallback((data: IDUpdateFormValues) => {
    setFormData(data);
    setStep("terms");
  }, []);
  ...
  const handleTermsAccepted = useCallback(async () => {
    ...
    if (formData.idFrontFile) fd.append("idFront", formData.idFrontFile);
    if (formData.idBackFile) fd.append("idBack", formData.idBackFile);
    if (formData.utilityBillFile) fd.append("utilityBill", formData.utilityBillFile);
        ...
        <Suspense fallback={<Loader text="Loading..." />}>
          ...
          {step === "verification" && (
            <VerificationStep
              ...
              serviceTitle="Identity Document Update"
            />
          )}
          {step === "form" && (
            <IDUpdateFormStep
